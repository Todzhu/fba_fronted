...
<template>
  ...
  <MessageModal title="全部消息" :fullscreen-button="false" class="w-[600px]">
    <div class="max-h-[500px] overflow-y-auto">
      <div v-if="allMessages.length === 0 && !allMessagesLoading" class="py-10 text-center text-gray-400">
        暂无消息
      </div>
        ...
        >
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium" :class="{ 'text-primary': !item.isRead }">{{ item.title }}</span>
              <span 
                v-if="!item.isRead" 
            ...
            <p class="mt-1 text-sm text-gray-500 line-clamp-2">{{ item.message }}</p>
      ...
      <div v-if="allMessages.length > 0 && allMessages.length < allMessagesTotal" class="py-3 text-center">
        <button 
