...
const currentLogTask = ref<null | TaskStatusResponse>(null);
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'task_name'">
            <Typography.Text
              ...
              :editable="{
                onChange: (val: string) => handleTaskNameChange(record.id, val),
              }"
              class="task-name"
            />
          </template>
    ...
    >
      <div class="log-content">
        <div v-if="currentLogTask?.status === 'failed'" class="log-error">
          ...
          <pre class="log-text">{{
            currentLogTask?.error_message || '无错误信息'
          }}</pre>
        </div>
        <div
          v-else-if="currentLogTask?.status === 'completed'"
          class="log-success"
        >
        ...
        <div
          v-else-if="currentLogTask?.status === 'running'"
          class="log-running"
        >
        ...
        <div class="log-meta">
          <p>
            <strong>创建时间：</strong
            >{{ formatTime(currentLogTask?.created_at ?? null) }}
          </p>
          <p v-if="currentLogTask?.started_at">
            <strong>开始时间：</strong
            >{{ formatTime(currentLogTask?.started_at) }}
          </p>
          <p v-if="currentLogTask?.completed_at">
            <strong>完成时间：</strong
            >{{ formatTime(currentLogTask?.completed_at) }}
          </p>
        </div>
      </div>
    </Modal>
