...
export function generateQcFilterResult(
  ...
  const filteredGenes = Math.floor(32_738 * (0.6 + Math.random() * 0.1));

  return {
    stats: {
      n_cells_before: 2700,
      n_cells_after: filteredCells,
      n_genes_before: 32_738,
      n_genes_after: filteredGenes,
      cells_removed: 2700 - filteredCells,
      genes_removed: 32_738 - filteredGenes,
    },
    charts: {
      qc_violin: {
...
export function generateAnnotationResult(
  ...
  const umapData: { color: string; type: string; x: number; y: number }[] = [];
