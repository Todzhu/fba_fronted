...
const triggerRdsPreview = async (fileUrl: string) => {
  ...
  try {
    ...
    console.log('[RDS] raw resp:', JSON.stringify(resp?.data).substring(0, 300));
    console.log('[RDS] resp.status:', resp?.status);
    ...
    console.error('[RDS] preview failed:', e?.message || e, e?.response?.status, e?.response?.data);
  }
...
const handleBinaryImportForKey = async (key: string, file: File) => {
  ...
  const isRds = ext === 'rds' || ext === 'rdata' || ext === 'rda' || ext === 'h5ad';
  ...
  if (isRds) {
    try {
      ...
      console.log('uploadTempFile response:', JSON.stringify(res));
      // uploadTempFile 返回 temp_path，直接传给后端
      const tempPath = (res as any)?.temp_path;
      if (tempPath) {
...
const handlePlatformFileSelect = async (file: any) => {
  ...
  // 如果是 RDS 文件，用 file.id 构造下载 URL 触发 metadata 预览
  if ((ext === 'rds' || ext === 'rdata' || ext === 'rda' || ext === 'h5ad') && file.id) {
    ...
    try {
      ...
      rdsPreviewLoading.value = false;
      message.success({ content: `${file.name} 已选择`, key: 'platformFilePreview' });
    }
  } else {
...
defineExpose({ fillAllExamples, getFileContents, setFileContents, getFileUrls, getFileIds, rdsPreview });
